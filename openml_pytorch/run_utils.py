...
def add_experiment_info_to_run(run, trainer, upload_model=False):
    """
    ...
    if upload_model:
        run = add_torch_model_to_run(run, trainer)
...
def add_torch_model_to_run(run, trainer):
    ...
    if os.path.exists(file_name):
        i = 1
        while os.path.exists(f"model_{i}.pth"):
            i += 1
        file_name = f"model_{i}.pth"
    torch.save(trainer.model.state_dict(), file_name)
    run = add_file_to_run(run, open(file_name, "rb"), file_name)
    return run
